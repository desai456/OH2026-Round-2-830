...
export interface QuoteItemData {
  ...
  line_total: number;
  customer_requested_discount_pct?: number | null;
  customer_requested_qty?: number | null;
}

export interface LineCommentData {
  id: string;
  quotation_line_id?: string | null;
  author_type: 'INTERNAL' | 'CUSTOMER' | string;
  author_name: string;
  comment_text: string;
...
export default function CustomerPortal({
  ...
  const [quoteData, setQuoteData] = useState<PortalQuoteData | null>(null);
  ...
  const [lineCounterDiscounts, setLineCounterDiscounts] = useState<Record<string, number>>({});
  const [lineCounterQtys, setLineCounterQtys] = useState<Record<string, number>>({});
  const [globalComment, setGlobalComment] = useState<string>("Proposing volume adjustment for enterprise expansion.");
  ...
  const fetchPortalQuote = async () => {
    ...
    try {
      ...
      const data: PortalQuoteData = await res.json();
      setQuoteData(data);

      // Initialize counter inputs
      const initDiscounts: Record<string, number> = {};
  ...
  const handleLineCommentSubmit = async (lineId: string) => {
  ...
  if (loading) {
    return (
      <div className="p-12 text-center text-[#A6A39C] flex items-center justify-center gap-3 bg-[#0A0A0B] min-h-screen font-sans">
