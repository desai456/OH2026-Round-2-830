...
export function QuoteSummaryCard({ quote, onPreview }: QuoteSummaryCardProps) {
  ...
  return (
    <Card className="border-slate-200/90 dark:border-slate-800 shadow-md sticky top-20">
      ...
      <div className="py-4 space-y-3 text-xs border-b border-slate-200 dark:border-slate-800">
        <div className="flex justify-between text-slate-600 dark:text-slate-400">
          <span>Catalog Subtotal</span>
          <span className="font-semibold text-slate-900 dark:text-slate-100">${metrics.subtotal.toLocaleString()}</span>
        </div>
        ...
        <div className="flex justify-between text-slate-600 dark:text-slate-400">
          ...
          <span className="font-semibold text-slate-900 dark:text-slate-100">+${metrics.totalTax.toLocaleString()}</span>
        </div>

        <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 flex justify-between font-bold text-sm text-slate-900 dark:text-slate-100">
      ...
      <div className="py-4 space-y-3 border-b border-slate-200 dark:border-slate-800">
        <div className="p-3 rounded-xl bg-slate-900 text-slate-100 dark:bg-slate-800 dark:text-slate-100 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Total Contract Value</span>
        ...
        <div className="flex justify-between items-center text-xs">
          <span className="text-slate-500 font-medium">Estimated Gross Margin:</span>
          <div className="text-right">
            <span className="font-bold text-slate-900 dark:text-slate-100">${metrics.grossMargin.toLocaleString()}</span>
