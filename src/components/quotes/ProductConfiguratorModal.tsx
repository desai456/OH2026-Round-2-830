  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Configure ${product.name}`} maxWidth="lg">
      <div className="space-y-5">
        ...
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
          <div>
            <span className="text-slate-500 block">Base Price: ${product.basePrice.toLocaleString()}</span>
            <span className="text-blue-600 dark:text-blue-400 font-semibold block mt-0.5">
              Config Adjustment: +${adj.toLocaleString()}
            </span>
          </div>
          <div className="text-right">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Calculated Unit Price</span>
            <span className="text-lg font-black text-slate-900 dark:text-slate-100">${finalUnitPrice.toLocaleString()}</span>
          </div>
        </div>
